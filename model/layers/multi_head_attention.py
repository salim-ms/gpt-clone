...
class MultiHeadAttention(nn.Module):
    def __init__(self, embed_size, n_heads, head_size, sequence_length, dropout=0.0):
        super(MultiHeadAttention, self).__init__()
        # create a list of multiple heads
        self.multi_heads = nn.ModuleList([MyAttentionHead(embed_size=embed_size, head_size=head_size, sequence_length=sequence_length) for _ in range(n_heads)])
...
if __name__ == "__main__":
    ...
    multi_head_attention = MultiHeadAttention(embed_size=16, n_heads=4, sequence_length=8, head_size=4)
